...
Public Class Frmmain
    ...
    Private Sub list_AfterAddItem(ByVal row As System.Data.DataRow) Handles list.AfterAddItem, list.AfterEditItem
        If IO.File.Exists(Clsql.Xml.sysdata("rpts") & row("code") & ".xml") Then
            IO.File.Delete(Clsql.Xml.sysdata("rpts") & row("code") & ".xml")
        End If
        Dim dtgridinfo As DataTable = conn.GetDatatable("select * from rpts where code ='" & row("code") & "'")
        dtgridinfo.WriteXml(Clsql.Xml.sysdata("rpts") & row("code") & ".xml", XmlWriteMode.WriteSchema)
        dtgridinfo.Dispose()
        'luu logo
        If IO.File.Exists(Clsql.Xml.sysdata("rpts") & row("code") & row("stt") & ".xml") Then
            IO.File.Delete(Clsql.Xml.sysdata("rpts") & row("code") & row("stt") & ".xml")
        End If
        dtgridinfo = conn.GetDatatable("select * from rpts where code ='" & row("code") & "' and stt ='" & row("stt") & "'")
        dtgridinfo.TableName = row("code") & row("stt")
        dtgridinfo.WriteXml(Clsql.Xml.sysdata("rpts") & row("code") & row("stt") & ".xml", XmlWriteMode.WriteSchema)
        dtgridinfo.Dispose()
    End Sub
