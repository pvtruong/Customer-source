...
Public Class frminput
    ...
    Private Sub btntao_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btntao.Click
        If checkkc() = False Then
            Exit Sub
        End If
        Dim dttable As DataTable = conn.GetDatatable("exec taobuttoanluong " & txtden_ky.Value & "," & txtnam.Value)
    ...
    Function checkkc() As Boolean
        If Voucher.Action = ClsSV31.Voucher20.Actions.ADD Then
            If conn.GetValue("select count(stt_rec) from socai where ma_ct ='" & Voucherid & "' and month(ngay_ct) =" & txtden_ky.Value & " and year(ngay_ct) =" & txtnam.Value & " and ma_dvcs ='" & Clsql.Reg.GetValue("unit") & "'") > 0 Then
                MsgBox(Voucher.oLan("K02"), MsgBoxStyle.YesNo, Me.Text)
                Return False
            End If
        End If
