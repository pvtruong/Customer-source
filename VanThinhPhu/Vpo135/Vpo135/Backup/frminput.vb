...
Public Class frminput
    ...
    Private Sub frminput_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        ...
        Next
        RemoveHandler Voucher.FormLookups("txtso_hd").AfterLookup, AddressOf laydulieutuhopdong
    End Sub
    ...
    Private Sub frminput_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        AddHandler Voucher.FormLookups("txtso_hd").AfterLookup, AddressOf laydulieutuhopdong
    End Sub
...
#End Region
    ...
    Private Sub btnLayDuLieuTuPhieuYC_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLayDuLieuTuPhieuYC.Click
        Dim layhd As New GetDataByStoreMD(Voucher.conn, "mpq1", "dpq1_get4po1", , "GetPQ1")
        ...
        If fdkloc.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
            If layhd.Show("Exec GetPQ14PO1 " & conn.ConvertToSQLType(fdkloc.Txttu_ngay.Value) & "," & conn.ConvertToSQLType(fdkloc.TxtDen_ngay.Value) & "," & conn.ConvertToSQLType(Voucher.Stt_rec)) = Windows.Forms.DialogResult.OK Then
    ...
    Private Sub laydulieutuhopdong(ByVal row As StpTableRow)
        ...
        Dim query As String = "select * from vdpo1 where so_ct='" & row("so_ct") & "'"
    ...
    Private Sub btngetso1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btngetso1.Click
        Dim layhd As New GetDataByStoreMD(Voucher.conn, "mso1", "dso1_get4po1", , "GetSO1")

        Dim fdkloc As New frmGetSO1
        ClsControl2.PropertyOfForm.SetLable(Voucher.oLan, fdkloc)
        fdkloc.Icon = Me.Icon
        fdkloc.StartPosition = FormStartPosition.CenterParent
        If fdkloc.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
            If layhd.Show("Exec GetSO14PO1 " & conn.ConvertToSQLType(fdkloc.Txttu_ngay.Value) & "," & conn.ConvertToSQLType(fdkloc.TxtDen_ngay.Value) & "," & conn.ConvertToSQLType(fdkloc.txtma_kh.Text) & "," & conn.ConvertToSQLType(fdkloc.txtso_so1.Text)) = Windows.Forms.DialogResult.OK Then
