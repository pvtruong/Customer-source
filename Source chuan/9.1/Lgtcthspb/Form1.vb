...
Public Class Frmmain
    ...
    Private Sub dmload(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If dkloc.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
            ...
            list.CSearch = "ky =" & ky & " and nam =" & nam
            If dkloc.chbcopy.Checked Then
                Dim ky_truoc As Integer = ky - 1
                Dim nam_truoc As Integer = nam
                If ky_truoc = 0 Then
                    ky_truoc = 12
                    nam_truoc = nam - 1
                End If
                Dim ds As DataTable = conn.GetDatatable("select * from " & list.tablename & " where " & "ky =" & ky_truoc & " and nam =" & nam_truoc)
                For Each r As DataRow In ds.Rows
                    r("ky") = ky
                    r("nam") = nam
                Next
                ds.AcceptChanges()
                Dim query As String = conn.GetInsertQueryFromDatatable(ds, list.tablename)
                query = "delete from " & list.tablename & " where " & "ky =" & ky & " and nam =" & nam & Chr(13) & query
                conn.Execute(query)
