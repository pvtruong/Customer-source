...
#End Region
    ...
    Function importInvVNPT(stt_rec As String) As String
        ...
        End If
        If conn.GetValue("select status from mhd2 where stt_rec='" & stt_rec & "'") = 9 Then
        ...
        invoice_string = invoice_string & Chr(13) & "<PaymentMethod>" & conn.GetValue("select hinh_thuc_tt from mhd2 where stt_rec='" & stt_rec & "'").ToString.Trim & "</PaymentMethod>"
        invoice_string = invoice_string & Chr(13) & "<Buyer>" & conn.GetValue("select ong_ba from mhd2 where stt_rec='" & stt_rec & "'").ToString.Trim & "</Buyer>"
        'detail
        ...
        If kq.Contains("ERR") Then
            conn.Server.writeError(invoice_string)
            Throw New Exception(kq)
        Else
            'Dim ma_hoa_don As String = kq.Split(";")(0).Split(":")(1)
            conn.Execute("update mhd2 set status='9' where stt_rec='" & stt_rec & "'")
        End If
        Return kq
    End Function
    ...
    Sub postimportfromexcel(ByVal dicstt_rec As Dictionary(Of String, String), ByVal status As Integer)
        For Each stt_rec As String In dicstt_rec.Keys
            conn.Execute("update mhd2 set ma_so_thue =b.ma_so_thue from mhd2 a join dkh b on a.ma_kh = b.ma_kh where a.ma_so_thue = '' and a.stt_rec ='" & stt_rec & "'")
            conn.Execute("update mhd2 set dia_chi =b.dia_chi from mhd2 a join dkh b on a.ma_kh = b.ma_kh where a.dia_chi = '' and a.stt_rec ='" & stt_rec & "'")

            conn.Execute("update vatra set ma_so_thue =b.ma_so_thue from vatra a join dkh b on a.ma_kh = b.ma_kh where a.ma_so_thue = '' and a.stt_rec ='" & stt_rec & "'")
            conn.Execute("update vatra set dia_chi =b.dia_chi from vatra a join dkh b on a.ma_kh = b.ma_kh where a.dia_chi = '' and a.stt_rec ='" & stt_rec & "'")

            'post(stt_rec, status)
            ClsSV31.Functions.Post(conn, Voucherid, Voucher.tenbangchung, Voucher.Stt_rec, status)
            'tao phieu thu
            frmin.tao_phieu_thu(stt_rec)
        Next
    End Sub
