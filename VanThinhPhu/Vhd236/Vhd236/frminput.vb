...
#End Region
    ...
    Function save() As Boolean
            ...
            If row.IsNewRow = False Then
                If row.Cells("ma_lo").Value = "" Then
                    ...
                    If conn.GetValue(queryg) = True Then
                        MsgBox("Không bỏ trống mã lô", MsgBoxStyle.Critical, Clsql.AboutMe.Name)
                        Return False
                    End If
                End If
            End If
        ...
        End If
        'update so hoa don
        Dim stt_rec_c As String = conn.GetValue("select stt_rec from vatra where so_hd ='" & txtso_hd.Text & "'")
        If Not (stt_rec_c = "" OrElse stt_rec_c = Voucher.Stt_rec) Then
            MsgBox("Số hóa đơn này đã tồn tại",, Clsql.AboutMe.Name)
            Return False
        End If

        If sohd IsNot Nothing AndAlso sohd = txtso_hd.Text Then
            vno.UpdateCode(sohd)
        End If
        ...
        If fdkloc.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
            If layhd.Show("Exec GetPXH4HD2 " & conn.ConvertToSQLType(fdkloc.Txttu_ngay.Value) & "," & conn.ConvertToSQLType(fdkloc.TxtDen_ngay.Value) & "," & conn.ConvertToSQLType(Voucher.Stt_rec) & "," & conn.ConvertToSQLType(fdkloc.txtma_nvu.Text)) = Windows.Forms.DialogResult.OK Then
                ...
                For Each r As DataRow In layhd.MDataView.Table.Select("sel = true")
                    ...
                    Voucher.CurrentVoucher("ngay_giao") = r("ngay_giao")
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        If vno Is Nothing Then
            vno = New ClsVno.AutoCreateCode(conn, "HD22")
        End If

        sohd = vno.GetNextCode("mhd2", "so_hd")
        Voucher.CurrentVoucher("so_hd") = sohd
        If Voucher.Tabdetails("vatra").Datatable.Rows.Count > 0 Then
            Voucher.Tabdetails("vatra").Datatable.Rows(0)("so_hd") = sohd
        End If
    End Sub
    ...
    Dim so_hd As String, ngay_hd As Date
    Dim so_seri As String
