...
#End Region
    ...
    Function save() As Boolean
        ...
        '
        tao_phieu_chi(voucher.Stt_rec)
        '
        Return True
    End Function

    Public Sub tao_phieu_chi(stt_rec As String)
        Dim query As String = ""
        'xoa phieu chi cu
        Dim dtpc As DataTable = conn.GetDatatable("select * from mpt1 where stt_rec_goc ='" & stt_rec & "'")
        If dtpc.Rows.Count > 0 Then
            Dim stt_rec_pc As String = dtpc.Rows(0).Item("stt_rec")
            query = "delete from dpt1 where stt_rec='" & stt_rec_pc & "'"
            query = query & ChrW(13) & "GO" & ChrW(13) & "delete from mpt1 where stt_rec='" & stt_rec_pc & "'"
        ...
        'tu tao phieu chi
        query = "exec CreatePTFromHD1 '" & stt_rec & "'"
        Dim dspc As DataSet = conn.GetDataSet(query)
        If dspc.Tables(0).Rows.Count > 0 Then
            ...
            conn.ExecuteWithMultiQuery(query)
            'post phieu chi
            ClsSV31.Functions.Post(conn, dspc.Tables(0).Rows(0).Item("ma_ct"), "vmpt1", dspc.Tables(0).Rows(0).Item("stt_rec"), dspc.Tables(0).Rows(0).Item("status"))
        End If
    End Sub
    Sub tinh_tong_tien()
        If isloaded = False Then
