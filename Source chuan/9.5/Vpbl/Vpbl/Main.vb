Module Main
    ...
    Public ma_dt As String = ""
    Public ma_phi As String = ""
    Public showProducts As Boolean = False
    ...
    Public Sub main()
        ...
        Dim odt As New ClsLookup.AutoCompleteLookup(conn, "dmdt", fchonca.txtma_dt, "ma_dt", False)
        odt.SetValue("ten_dt", fchonca.ten_dt)

        Dim ophi As New ClsLookup.AutoCompleteLookup(conn, "dmphi", fchonca.txtma_phi, "ma_phi", False)
        ophi.SetValue("ten_phi", fchonca.ten_phi)
        ...
        If fchonca.ShowDialog = Windows.Forms.DialogResult.OK Then
            ...
            ma_dt = fchonca.txtma_dt.Text
            ma_phi = fchonca.txtma_phi.Text
            showProducts = fchonca.chbshowProducts.Checked
        Else
        ...
        End If
        conn.Execute("exec TinhLaiTonKHoTucThoi " & Now.Year & ", '" & ma_kho & "',''")
        Dim fmain As New frmMain
        fmain.TopSplitContainer.Panel1Collapsed = Not showProducts
        fmain.setup()
        fmain.ShowDialog()
