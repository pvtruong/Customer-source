...
#End Region
    ...
    Private Sub cbbma_nt_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbbma_nt.SelectedIndexChanged
        If isloaded = False Then
            Exit Sub
        End If
        Try
            ...
            If cbbma_nt.SelectedItem IsNot Nothing Then
                ...
                Dim ma_nt As String = dvr("ma_nt")

                Dim ty_gia = conn.GetValue("exec getty_gia " & conn.ConvertToSQLType(ma_nt) & "," & conn.ConvertToSQLType(Txtngay_ct.Value))
                Txtty_gia.Value = ty_gia
        ...
        If fdkloc.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
            If layhd.Show("Exec GetPO14pn9 " & conn.ConvertToSQLType(fdkloc.Txttu_ngay.Value) & "," & conn.ConvertToSQLType(fdkloc.TxtDen_ngay.Value) & "," & conn.ConvertToSQLType(Voucher.Stt_rec)) = Windows.Forms.DialogResult.OK Then
                ...
                For Each r As DataRow In layhd.MDataView.Table.Select("sel = true")
                    ...
                    Voucher.CurrentVoucher("ten_kh") = r("ten_kh")
                    Voucher.CurrentVoucher("ma_nt") = r("ma_nt")
                    Voucher.CurrentVoucher("ty_gia") = r("ty_gia")
                    Voucher.CurrentVoucher("ong_ba") = r("ong_ba")
                    txtma_kh.Focus()
                    Voucher.Tabdetails(Voucher.TabFirst).gridDetailKeyin.Focus()
                    Exit For
                Next
                hide_show_control(Voucher.CurrentVoucher("ma_nt"))
                tinh_tong_tien()
