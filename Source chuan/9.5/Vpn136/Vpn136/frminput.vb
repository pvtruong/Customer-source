...
#End Region
    ...
    Sub Phan_bo_chi_phi()
        ...
        If t_tien <> 0 Then
            ...
            If row_first IsNot Nothing AndAlso cp_nt <> txtt_cp_nt.Value Then
                ...
                row_first.Cells("tien_nhap_nt").Value = row_first.Cells("tien_hang_nt").Value + row_first.Cells("tien_phi_nt").Value
    ...
    Private Sub LayDuLieuTuPhieuNhapHangToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LayDuLieuTuPhieuNhapHangToolStripMenuItem.Click
        ...
        fdkloc.StartPosition = FormStartPosition.CenterParent
        If fdkloc.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
            If layhd.Show("Exec GetPNH4PN1 " & conn.ConvertToSQLType(fdkloc.Txttu_ngay.Value) & "," & conn.ConvertToSQLType(fdkloc.TxtDen_ngay.Value) & "," & conn.ConvertToSQLType(Voucher.Stt_rec) & "," & conn.ConvertToSQLType(fdkloc.txtma_nvu.Text)) = Windows.Forms.DialogResult.OK Then
                For Each r As DataRow In layhd.MDataView.Table.Select("sel = false")
                    For Each r2 As DataRow In layhd.DDataView.Table.Select("stt_rec = '" & r("stt_rec") & "'")
                        r2.Delete()
                    Next
