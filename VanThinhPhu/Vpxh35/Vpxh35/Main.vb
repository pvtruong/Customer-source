...
Public Class Frmmain
    ...
    Dim value_p As Object = Nothing
    Private Sub grd_CellEnter(sender As Object, e As DataGridViewCellEventArgs) Handles grd.CellEnter
        If grd.CurrentRow IsNot Nothing AndAlso e.RowIndex >= 0 Then
            value_p = grd.CurrentRow.Cells(e.ColumnIndex).Value
        End If
    End Sub

    Private Sub grd_CellValidated(sender As Object, e As DataGridViewCellEventArgs) Handles grd.CellValidated
        Dim query As String
        If grd.Columns(e.ColumnIndex).Name = "ngay_giao" AndAlso grd.CurrentRow.Cells("ngay_giao").ReadOnly = False Then
            grd.EndEdit()
            If IsDBNull(value_p) AndAlso IsDBNull(grd.CurrentRow.Cells("ngay_giao").Value) Then
                Return
            End If
            ...
            End If
            'kiem tra quyen sua
            query = "select value1 from options where name ='user_ngay_giao' and dbo.einstr('" & Reg.GetValue("id") & "',value1)=1"
            If conn.GetValue(query) = "" Then
                grd.CurrentRow.Cells("ngay_giao").Value = value_p
                grd.EndEdit()
                MsgBox("Bạn không có quyền sửa thông tin này",, Clsql.AboutMe.Name)

                Return
            End If
            'kiem tra phat sinh
            query = "select top 1 stt_rec_pxh as test from dhd2 where stt_rec_pxh =" & conn.ConvertToSQLType(grd.CurrentRow.Cells("stt_rec").Value)
            If conn.GetValue(query) = "1" Then
                grd.CurrentRow.Cells("ngay_giao").Value = value_p
                grd.EndEdit()
                MsgBox("KHông thể sửa phiếu xuất hàng này do đã được xuất hóa đơn", "", Clsql.AboutMe.Name)
                Return
            End If
            query = "update mpxh set ngay_giao =" & conn.ConvertToSQLType(grd.CurrentRow.Cells("ngay_giao").Value) & " where stt_rec=" & conn.ConvertToSQLType(grd.CurrentRow.Cells("stt_rec").Value)
            conn.Execute(query)
        End If
    End Sub
