...
#End Region
    ...
    End Sub
    Dim ds As DataSet
    Private Sub rpt_AfterGetData(dataset As DataSet) Handles rpt.AfterGetData
        ds = dataset
        For Each c As DataGridViewColumn In grd.Columns
            If Not ds.Tables(0).Columns.Contains(c.Name) Then
                For Each r As DataGridViewRow In grd.Rows
                    If Not IsDBNull(r.Cells(c.Name).Value) Then
                        Dim v As Double = r.Cells(c.Name).Value

                        If v >= 1 AndAlso v <= 7 Then
                            ClsControl2.PropertyOfGrid.FormatCell(r.Cells(c.Name), grd.Font.Size, FontStyle.Regular, Color.Green)

                        End If
                        If v > 7 AndAlso v <= 14 Then
                            ClsControl2.PropertyOfGrid.FormatCell(r.Cells(c.Name), grd.Font.Size, FontStyle.Regular, Color.YellowGreen)

                        End If
                        If v > 14 Then
                            ClsControl2.PropertyOfGrid.FormatCell(r.Cells(c.Name), grd.Font.Size, FontStyle.Regular, Color.Red)
