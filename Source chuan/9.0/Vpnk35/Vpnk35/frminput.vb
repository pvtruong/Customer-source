...
#End Region
    ...
    Private Sub TạoPhiếuNhậpKhoTừPhiếuNhậpHàngToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles TạoPhiếuNhậpKhoTừPhiếuNhậpHàngToolStripMenuItem.Click
        Dim layhd As GetDataByStoreMD
        ...
        fdkloc.StartPosition = FormStartPosition.CenterParent
        If fdkloc.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
            If layhd.Show("Exec GetPNH4PNK " & conn.ConvertToSQLType(fdkloc.Txttu_ngay.Value) & "," & conn.ConvertToSQLType(fdkloc.TxtDen_ngay.Value) & "," & conn.ConvertToSQLType(Voucher.Stt_rec) & "," & conn.ConvertToSQLType(fdkloc.txtma_nvu.Text)) = Windows.Forms.DialogResult.OK Then
                For Each r As DataRow In layhd.MDataView.Table.Select("sel = false")
                    For Each r2 As DataRow In layhd.DDataView.Table.Select("stt_rec = '" & r("stt_rec") & "'")
                        r2.Delete()
                    Next
