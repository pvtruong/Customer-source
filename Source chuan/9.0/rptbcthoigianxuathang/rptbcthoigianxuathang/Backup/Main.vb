...
#Region "form dkloc"
    Sub setupdkloc()
        Dim otk_vt As New ClsLookup.AutoCompleteLookup(rpt.conn, "dmtk", frmdkloc.txttk_vt, "tk", False)
        Dim otk_du As New ClsLookup.AutoCompleteLookup(rpt.conn, "dmtk", frmdkloc.txttk_du, "tk", False)
        ...
        If Clsql.Reg.GetValue("Language") = "Vi" Then
            ...
            ClsLookup.AddItems.AddItemCbbGotBlank(rpt.conn, "select codeid,headerV as txt from optgroupby where type =2 and rptid ='" & rptid & "'", frmdkloc.cbbnhom, "txt", "codeid")
            otk_vt.SetValue("ten_tk", frmdkloc.ten_tk_vt)
            otk_du.SetValue("ten_tk", frmdkloc.ten_tk_du)
        ...
        Else
            ...
            ClsLookup.AddItems.AddItemCbbGotBlank(rpt.conn, "select codeid,headerE as txt from optgroupby where type =2 and rptid ='" & rptid & "'", frmdkloc.cbbnhom, "txt", "codeid")


            otk_vt.SetValue("ten_tk2", frmdkloc.ten_tk_vt)
            otk_du.SetValue("ten_tk2", frmdkloc.ten_tk_du)
        ...
        Try
            ...
            End If
            frmdkloc.txttk_vt.Text = Clsql.Reg.GetValue("dstk")
    ...
    Sub GetQuery()
        ...
        query = query & "," & rpt.conn.ConvertToSQLType(frmdkloc.chung_tu_dc.Checked)
        ...
        query = query & "," & rpt.conn.ConvertToSQLType(Clsql.Others.GetArgu(2))
        query = query & "," & rpt.conn.ConvertToSQLType(Clsql.Others.GetArgu(3))
        'xem chi tiet
        Dim xemchitiet As String = rpt.conn.ConvertToSQLType(frmdkloc.Txttu_ngay.Value)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.TxtDen_ngay.Value)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.txttk_vt.Text)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.txttk_du.Text)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.txtma_vt.Text)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.txtma_kh.Text)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.txtma_kho.Text)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.txtma_dt.Text)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.txtma_nv.Text)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.chung_tu_dc.Checked)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.cbbma_dvcs.SelectedValue)
        '  query = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.cbbbctheo.SelectedValue)
        ' xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.cbbnhom.SelectedValue)
        'query = query & "," & rpt.conn.ConvertToSQLType(rptid)
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(Clsql.Others.GetArgu(2))
        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(Clsql.Others.GetArgu(3))

        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.cbbbctheo.SelectedValue)
        xemchitiet = xemchitiet & ",'?" & frmdkloc.cbbbctheo.SelectedValue & "?'"

        xemchitiet = xemchitiet & "," & rpt.conn.ConvertToSQLType(frmdkloc.cbbnhom.SelectedValue)
        xemchitiet = xemchitiet & ",'?" & frmdkloc.cbbnhom.SelectedValue & "?'"
        rpt.DrillDown.convert2sql_yn = True
        rpt.DrillDown.setCondition("chitiet", xemchitiet)
...
#End Region
    ...
    Sub SetParas()
        ...
        rpt.cPrint.SetParameterValue("tk", frmdkloc.txttk_vt.Text)
